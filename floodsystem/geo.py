...
def rivers_by_station_number(stations, N):
    """Return the first N entries of the list of rivers by greatest number of stations. If there are more rivers with the same number of stations the Nth river, they will also be included in the output."""
    rivers_number = sorted_by_key([(river[0], len(river[1])) for river in stations_by_river(stations).items()], 1, reverse=True)
